        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('wuziFafang','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xinguanwuzi/upload/wuziFafangMuBan.xls"
                        >批量导入物资发放数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('wuziFafang','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="wuziFafangUploadSuccess"
                                :on-error="wuziFafangUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('wuziFafang','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入物资发放数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('wuziFafang','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "wuziFafang.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="wuziFafangUuidNumber"
                                   header-align="center"
                                   label="物资发放编号">
                        <template slot-scope="scope">
                            {{scope.row.wuziFafangUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuziFafangNumber"
                                      header-align="center"
                                      label="发放数量">
                        <template slot-scope="scope">
                            {{scope.row.wuziFafangNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="wuziFafangAddress"
                                   header-align="center"
                                   label="发放位置">
                        <template slot-scope="scope">
                            {{scope.row.wuziFafangAddress}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('wuziFafang','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('wuziFafang','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('wuziFafang','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                yuangongId : null,
                wuziId : null,
                wuziFafangUuidNumber : null,
                wuziFafangNumber : null,
                rukuTime : null,
                wuziFafangAddress : null,
                wuziFafangContent : null,
                wuziFafangBeizhuContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '物资发放编号': "wuziFafangUuidNumber",
                     '发放数量': "wuziFafangNumber",
                     '发放时间': "rukuTime",
                     '发放位置': "wuziFafangAddress",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'wuzi_fafang',//当前表表名,
                        sumColum : 'wuzi_fafang_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'wuzi_fafang_name',//分组字符串字段
                        // types : 'wuzi_fafang_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['wuziFafangDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            wuziFafangUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "wuziFafang/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "wuziFafang/update",
                        method: "post",
                        data: {
                            id:id,
//                            wuziFafangTypes:wuziFafangTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
