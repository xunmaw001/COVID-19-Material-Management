...
public class WuziYuyueController {
    ...
    public R save(@RequestBody WuziYuyueEntity wuziYuyue, HttpServletRequest request){
        ...
        WuziEntity wuziEntity = wuziService.selectById(wuziYuyue.getWuziId());
        if(wuziEntity == null)
            return R.error("查不到物资");
        int balance = wuziEntity.getWuziKucunNumber() - wuziYuyue.getWuziYuyueNumber();
        if(balance<0){
            return R.error("预约物资数量不能大于库存物资");
        }
        ...
        Wrapper<WuziYuyueEntity> queryWrapper = new EntityWrapper<WuziYuyueEntity>()
                ...
                .in("wuzi_yuyue_yesno_types", new Integer[]{1})
                ;
        ...
        if(wuziYuyueEntity==null){
            ...
            if(wuziYuyueEntity.getWuziYuyueYesnoTypes()==1)
                return R.error(511,"该用户该物资有待审核的记录,请等待审核后再申领");
            else
...
//            wuziYuyue.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    public R shenhe(@RequestBody WuziYuyueEntity wuziYuyueEntity, HttpServletRequest request){
        ...
        if(wuziYuyueEntity.getWuziYuyueYesnoTypes() == 2){//通过
            ...
            if(wuziEntity == null)
                return R.error("查不到物资");
            int balance = wuziEntity.getWuziKucunNumber() - oldWuziYuyue.getWuziYuyueNumber();
            if(balance<0)
                return R.error("无法同意申请,因为库存物资不足,请核实后再审核");
...
//                            wuziYuyueEntity.setWuziYuyueYesnoText(data.get(0));                    //申请回复 要改的
